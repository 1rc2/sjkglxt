...
下载页 `https://1rc2.github.io/` 已支持多应用顶部切换。给下载页新增一个应用时：

1. 编辑下载页仓库 `1rc2/1rc2.github.io` 的 `index.html`，在 `CONFIG.apps` 数组中复制一项；
2. 修改三项：`repo`（新应用所在 GitHub 仓库名）、`appName`（切换条显示名）、`color`（主题色）；
3. 提交并推送，顶部切换条自动出现新应用标签，无需改其他代码；
4. 要求：新应用仓库必须是公开仓库，且按「二、发布新版本流程」发布带 `.apk` 资产的 Release。
